...
### Log Parsing and Reporting Format

**CRITICAL:** Use the structured log format to avoid misreporting.
...
타임아웃: 60분
재시도 간격: 10초
대상 열차: 1,2,3,4,5,6,7,8,9 (총 9개)
시작 시간: 2026-02-03 14:11:54
============================================================

=== 시도 #1 (열차 1/9) ===
열차: 329 (120400 → 135600)
...
...
**Parsing Rules:**
1. Read **출발역** and **도착역** from header (lines after "SRT 예약 재시도 시작")
2. Read **시작 시간** from header
3. Find latest **진행 상황 요약** for elapsed/remaining time
4. Count **시도 #N** lines for attempt count
5. Check for **최종결과** (SUCCESS or TIMEOUT)
...
경과: {경과 시간}분 / {타임아웃}분
시도: #{현재 시도 횟수}
상태: {SUCCESS/FAILED/진행중}

[성공 시] ✅ 예약 성공! 열차 {열차번호}, 좌석 {좌석번호}
[실패 시] ❌ 모든 시도 좌석 없음
[타임아웃 시] ⏰ 타임아웃 ({타임아웃}분 경과)
```
...
**DO NOT:**
- Invent station names (always read from log)
- Guess elapsed time (calculate from 시작 시간 or read from 진행 상황 요약)
- Report without checking 최종결과 for completion
